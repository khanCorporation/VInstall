    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (status) {
            PackageInstaller.STATUS_SUCCESS -> {
                ...
                InstallHelper.emit(InstallHelper.Result.Success)
            }
            ...
            PackageInstaller.STATUS_FAILURE_ABORTED -> {
                ...
                InstallHelper.emit(InstallHelper.Result.Failure("Installation was cancelled"))
            }
            PackageInstaller.STATUS_FAILURE_BLOCKED -> {
                DebugLog.e("InstallReceiver", "Install blocked by the system")
                InstallHelper.emit(InstallHelper.Result.Failure("Installation blocked by the system (check Play Protect or device policy)"))
            }
            PackageInstaller.STATUS_FAILURE_CONFLICT -> {
                DebugLog.e("InstallReceiver", "Install conflict (signature mismatch or version downgrade)")
                InstallHelper.emit(InstallHelper.Result.Failure("Conflict: signature mismatch or version downgrade. Uninstall the existing app first."))
            }
            PackageInstaller.STATUS_FAILURE_INCOMPATIBLE -> {
                DebugLog.e("InstallReceiver", "Install incompatible (ABI or SDK mismatch)")
                InstallHelper.emit(InstallHelper.Result.Failure("Incompatible APK: ABI or Android version mismatch"))
            }
            PackageInstaller.STATUS_FAILURE_INVALID -> {
                DebugLog.e("InstallReceiver", "Install invalid APK")
                InstallHelper.emit(InstallHelper.Result.Failure("Invalid APK: the package file is malformed"))
            }
            PackageInstaller.STATUS_FAILURE_STORAGE -> {
                DebugLog.e("InstallReceiver", "Install failed due to insufficient storage")
                InstallHelper.emit(InstallHelper.Result.Failure("Insufficient storage space"))
            }
            else -> {
                DebugLog.e("InstallReceiver", "Install FAILED: $message (status=$status)")
                InstallHelper.emit(InstallHelper.Result.Failure(message ?: "Install failed (code $status)"))
            }
        }
    }
}
